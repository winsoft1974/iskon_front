...
export class YatrisComponent implements OnInit {
  ...
  isImportingDevotees = false;
  importProgressMessage = '';
  importProgressPercent = 0;

  async importDevoteesFromExcel(fileInputEvent?: any) {
    let dataBuffer: ArrayBuffer;
    this.isImportingDevotees = true;
    this.importProgressMessage = '📊 Excel फाईल वाचली जात आहे...';
    this.importProgressPercent = 0;

    try {
      if (fileInputEvent && fileInputEvent.target?.files?.[0]) {
        const file = fileInputEvent.target.files[0];
        dataBuffer = await file.arrayBuffer();
      } else {
        const resp = await fetch('/assets/devotees-list.xlsx');
        if (!resp.ok) throw new Error('assets/devotees-list.xlsx फाईल सापडली नाही.');
        const cType = resp.headers.get('content-type') || '';
        if (cType.includes('text/html')) {
          throw new Error('कृपया "devotees-list.xlsx Import करा (File निवडा)" वर क्लिक करून कॉम्प्युटरवरील devotees-list.xlsx फाईल निवडा.');
        }
      ...
      if (!rawRows || rawRows.length === 0) {
        alert('⚠️ Excel फाईल रिकामी आहे किंवा रेकॉर्ड्स सापडले नाहीत.');
        this.isImportingDevotees = false;
        return;
      }
      ...
      for (let i = 0; i < total; i++) {
        const row = rawRows[i];
        const memberPayload = {
          sno: row['Sno'] ? parseInt(row['Sno']) : null,
          did: row['DID'] ? parseInt(row['DID']) : null,
          name: String(row['Name'] || '').trim(),
          legalName: row['Legal Name'] ? String(row['Legal Name']).trim() : null,
          initiatedName: row['Initiated Name'] ? String(row['Initiated Name']).trim() : null,
          gender: row['G'] ? (String(row['G']).toLowerCase().startsWith('f') ? 'Female' : 'Male') : 'Male',
          dtBirth: parseDateVal(row['DtBirth']),
          ashram: row['Ashram'] ? String(row['Ashram']).trim() : null,
          centerName: row['Center-name'] ? String(row['Center-name']).trim() : 'ISCON',
          centerState: row['Center-state'] ? String(row['Center-state']).trim() : null,
          centerCountry: row['Center-country'] ? String(row['Center-country']).trim() : null,
          satsangas: row['Satsangas'] ? String(row['Satsangas']).trim() : null,
          spouseName: row["Spouse's Name"] ? String(row["Spouse's Name"]).trim() : null,
          dtMarriage: parseDateVal(row['DtMarriage']),
          mobiles: row['Mobiles'] ? String(row['Mobiles']).trim() : null,
          ...
          rounds: row['#Round'] !== undefined && row['#Round'] !== null ? String(row['#Round']) : null,
          sinceDt: parseDateVal(row['SinceDt']),
          durationYrMth: row['For(Yr.Mth)'] ? String(row['For(Yr.Mth)']).trim() : null,
          harinamDikshaguru: row['Harinam Diksguru'] ? String(row['Harinam Diksguru']).trim() : null,
          dtHarinamInitiation: parseDateVal(row['DtHarinamInitiation']),
          dtBrahminInitiation: parseDateVal(row['DtBrahminInitiation']),
          sannyasDikshaguru: row['Sanyas Diksaguru'] ? String(row['Sanyas Diksaguru']).trim() : null,
          dtSannyasInitiation: null,
          father: row['Father'] ? String(row['Father']).trim() : null,
          mother: row['Mother'] ? String(row['Mother']).trim() : null,
          deceasedDt: parseDateVal(row['DeceasedDt']),
          deceasedRemarks: row['DeceasedRemarks'] ? String(row['DeceasedRemarks']).trim() : null,
          photoUrl: row['photo-url'] ? String(row['photo-url']).trim() : null,
          photo: null
        };

        if (!memberPayload.name) continue;

        try {
          await this.api.create('IsconMembers', memberPayload).toPromise();
          successCount++;
        } catch (err) {
          failCount++;
        }

        this.importProgressPercent = Math.round(((i + 1) / total) * 100);
        this.importProgressMessage = `⏳ ${i + 1} / ${total} सदस्य डेटाबेसमध्ये सेव्ह होत आहेत... (${successCount} यशस्वी, ${failCount} अपयशी)`;
      }

      alert(`✅ Excel Import पूर्ण झाले!\nएकूण ${successCount} ISCON सदस्य यशस्वीपणे Database मध्ये जोडले गेले.`);
      ...
      alert('⚠️ Import करताना एरर आला: ' + (err.message || err));
    } finally {
      this.isImportingDevotees = false;
    }
  }
